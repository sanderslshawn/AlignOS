...
export default function SignalsScreen() {
  ...
  const signals = useMemo(() => normalizeQuickStatusSignals((dayState as any)?.quickStatusSignals), [dayState]);
  ...
  const toggleSignal = async (signal: QuickStatusSignal) => {
    ...
    setLastImpact(`${next.map((s) => QUICK_STATUS_LABELS[s]).join(', ') || 'none'}`);
    if (!signals.includes(signal)) void fetchSignalRecommendation(signal);
  };
